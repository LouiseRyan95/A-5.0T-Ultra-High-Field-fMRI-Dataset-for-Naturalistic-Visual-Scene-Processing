...
|---|---|---|---|
| `scripts/00_experimental_paradigm/nvd_naturalistic_visual_scene_task.m` | Run the Psychtoolbox naturalistic visual scene task. | image directory, subject/session arguments | BIDS-like `events.tsv` and task log |
| `scripts/00_preprocessing/run_fmriprep_docker.sh` | Record and run the fMRIPrep Docker preprocessing command. | BIDS root, output directory, work directory, FreeSurfer license | fMRIPrep derivatives including 91k CIFTI outputs |
